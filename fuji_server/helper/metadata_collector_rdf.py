...
class MetaDataCollectorRdf(MetaDataCollector):
    ...
    def get_metadata_from_graph(self, rdf_response_graph):
        ...
        if rdf_response_graph:
            ...
            if isinstance(rdf_response_graph, rdflib.graph.Graph) or isinstance(
                ...
                if (
                    rdflib.term.URIRef("http://data.europa.eu/930/")
                    in graph_namespaces.values()
                ):
                    self.logger.info(
                        "FsF-F2-01M : RDF Graph seems to contain GeoDCAT-AP metadata elements"
                    )
    ...
    def get_geodcat_metadata(self, graph):
        ...
        self.logger.info("FsF-F2-01M : Trying to get some GeoDCAT properties")
        ...
        if datasets:
            ...
            if spatial_info:
                ...
                geodcat_metadata["reference_system"] = reference_system
    ...
    def get_spatial_resolutions_dict(self, graph, dataset):
        ...
        spatial_resolutions = {
            ...
            for key, prop in resolution_types.items()
    ...
    def get_reference_system_dict(self, graph, dataset):
        ...
        print(f"GEODCAT.referenceSystem: {GEODCAT.referenceSystem}")
        return {"reference_system_uri": GEODCAT.referenceSystem}

    def is_epsg_listed(vocabulary_uri: str, epsg_code: int) -> bool:
        """Reference source: http://www.opengis.net/def/crs/EPSG/0/

        Args:
            epsg_code (int): _description_

        Returns:
            bool: _description_
        """
        # Construct the URL for the given EPSG code
        url = f"http://www.opengis.net/def/crs/EPSG/0/{epsg_code}"

        try:
            # Send a GET request to the URL
            response = requests.get(url)

            # If the response status code is 200, the EPSG code exists
            if response.status_code == 200:
                return True
            else:
                return False
        except requests.RequestException as e:
            # Handle any network errors or request issues
            print(f"Error checking EPSG code {epsg_code}: {e}")
            return False

    def extract_epsg_code(url: str) -> int:
        """_summary_
        ...
        Returns:
            int: _description_
        """
        # Define a regular expression pattern to match EPSG code in the URL
        pattern = r"https://www\.opengis\.net/def/crs/EPSG/0/(\d+)"
        ...
        if match:
            ...
            # If the pattern is not found, return None or handle as needed
            print("Invalid URL or EPSG code not found.")
            return None
